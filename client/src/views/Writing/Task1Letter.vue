...
<template>
  <div class="task1-page">
    <van-nav-bar title="Task 1 - Letter" left-text="Back" left-arrow fixed placeholder @click-left="$router.back()" />
      ...
      <van-skeleton title :row="3" :loading="loading">
        <div v-if="question" class="task-content">
          ...
          <div class="prompt-card">
            <van-tag type="primary" size="medium">{{ question.questionType }}</van-tag>
            <div class="prompt-text">{{ question.writingPrompt }}</div>
          </div>

          <!-- Writing Area -->
          <div class="writing-area">
            <div class="word-count">Words: {{ wordCount }} / 150 (min)</div>
            <van-field
              v-model="essay"
              type="textarea"
              rows="12"
